<template>
	<view class="message">
		<u-navbar back-text="返回" back-icon-size="0" title="我的通知" :back-text-style="backTextStyle" title-width="300" height='44' title-color="#000000">
		</u-navbar>
		<view v-show="item.state==2" class="content" v-for="(item, index) in list" :key="index">
		...
		<view v-show="item.state==1" class="contentno" v-for="(item, index) in list" :key="index">
			<view @click="listMessage()" class="list">
				<view style="width: 14rpx;height: 14rpx;border-radius: 50%;background-color: #ff5644;"></view>
				<view class="title">{{item.title}}</view>
				<view class="time">{{item.begintime}}</view>
